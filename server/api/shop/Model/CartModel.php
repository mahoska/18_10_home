...
public function getCartByClient($client_id)
{
    try{
        $sth = $this->pdo->prepare(
                "SELECT book.id, book.name, book.price, book.discount as book_discount, count, client.discount as  client_discount "
...
public function deleteCart($book_id){
    try{
        $sth = $this->pdo->prepare("DELETE FROM cart WHERE  book_id=:book_id");
